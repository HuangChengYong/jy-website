package com.jyinfo.jy_utils.DateUtil;
...
public class DateUtil {
     ...
     * @return 日期
     */
    public static String LongToString(Long millisecondStamp, DateStyle dateStyle) {
        String dateString = null;
        if (dateStyle != null) {
            dateString = DateToString(new Date(millisecondStamp), dateStyle.getValue());
        }
        return dateString;
    }
     ...
     * @return
     */
    public static List<Date> getRangeDates(Date dBegin, Date dEnd) {  
     ...
     * @return
     */
    public static String dateDifferentString(Date dateStart,Date dateEnd){
        StringBuilder differentString = new StringBuilder();
        try {
            if((dateStart != null) && (dateEnd != null)) {
                DateTime dt1 = new DateTime(dateStart);
                DateTime dt2 = new DateTime(dateEnd);

                int days = Days.daysBetween(dt1, dt2).getDays();
                if (days > 0) {
                    differentString.append(days + "天");
                }
                int hours = Hours.hoursBetween(dt1, dt2).getHours() % 24;
                if (hours > 0) {
                    differentString.append(hours + "小时");
                }
                int minutes = Minutes.minutesBetween(dt1, dt2).getMinutes() % 60;
                if (minutes > 0) {
                    differentString.append(minutes + "分钟");
                }
                int seconds = Seconds.secondsBetween(dt1, dt2).getSeconds() % 60;
                if (seconds > 0) {
                    differentString.append(seconds + "秒");
                }
                System.out.print("两个时间相差：" + differentString);
            }
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
